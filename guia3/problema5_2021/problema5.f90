...
REAL(pr),DIMENSION(0:n)              :: vecx,vecy
REAL(pr)                             :: x,Px,lx,delta
INTEGER                              :: i,Nmax=200
...
vecx=(/1._pr,1.1_pr,1.3_pr,1.4_pr/) !valores iniciales del vector y puntos por los que debe pasar el polinomio interpolantes 
        ...
        DO i=0,n
            vecy(i)=f(vecx(i))  !Los valores iniciales del vecy son las                                                     funciones evaluadas en esos puntos
!               WRITE(13,12) vecx(i),vecy(i)
            WRITE(14,12) vecx(i),vecy(i)
        END DO
...
delta=(vecx(n)-vecx(0))/(Nmax-1)  !Salto de paso en paso para evaluar el polinomio y que esten equiespaciados los puntos
        ...
        DO i=1,Nmax
            x=vecx(0)+(i-1)*delta
            CALL newton(n,vecx,vecy,x,Px)
!             WRITE(15,17) x,Px,f(x),(f(x)-Px)
            WRITE(16,17) x,Px,f(x),(f(x)-Px)
        END DO
